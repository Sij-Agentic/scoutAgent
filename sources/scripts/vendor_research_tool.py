...
class VendorResearchTool(Tool):
    ...
    def _analyze_with_llm(self, vendor_name: str, pain_point: str, content: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # Use DeepSeek model with synchronous completion
            response = completion(
                model="deepseek-chat",
    ...
    async def _research_vendor(self, vendor_name: str, pain_point: str, url: str = None) -> Dict[str, Any]:
                ...
                if scraped_data:
                    ...
                    if content:
                        ...
                        if llm_analysis:
                            ...
                            self.logger.warning("LLM analysis returned no results")
                    else:
        ...
        # Search for features and value proposition if not already found
        if not result['business_profile']['features'] or not result['business_profile']['value_proposition']:
            feature_query = f"{vendor_name} features and value proposition"
            feature_results = self._search_web(feature_query)
            if feature_results:
                for r in feature_results:
                    ...
                    # Extract features from the snippet if not already found
                    if not result['business_profile']['features'] and ('feature' in r['snippet'].lower() or 'includes' in r['snippet'].lower()):
                        features = [f.strip() for f in r['snippet'].split('.') if f.strip() and len(f.split()) < 10]
                        result['business_profile']['features'].extend(features[:5])
                    
                    # Extract value proposition if not already found
                    if not result['business_profile']['value_proposition'] and 'value' in r['snippet'].lower():
                        result['business_profile']['value_proposition'] = r['snippet'][:200] + '...'

...
mcp = FastMCP("web-search", host="127.0.0.1", port=8004)


async def _ddg_search(query: str, limit: int = 5) -> List[Dict[str, str]]:
    """
    Very simple DuckDuckGo HTML scraper to return a list of {title, url}.
    This avoids external API keys and works without auth.
    """
    q = quote_plus(query)
